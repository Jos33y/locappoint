...
export const openStatus = (week, timeZone) => {
    ...
    if (current) return { open: true, text: `Open now, until ${clock(current.end)}` }
    ...
    for (let i = 1; i <= 7; i++) {
        ...
        if (week[dow].length > 0) {
            ...
            return { open: false, text: `Closed now, opens ${day} at ${clock(week[dow][0].start)}` }
        }
    }
    return null
}
                        ...
                        {visible.map((service) => (
                            <li key={service.id || service.key} className="lc-pub__service">
                                ...
                                <span className="lc-pub__svcside">
                                    ...
                                    <Button onClick={onBook ? () => onBook(service) : undefined}>Book</Button>
                                </span>
                            </li>
                        ))}
                    </ul>
