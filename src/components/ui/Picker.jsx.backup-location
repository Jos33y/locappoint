...
export const Picker = ({
    id, value, onChange, options, placeholder = 'Choose one', title, popular = [], searchable = false,
    ...
    return (
        <div ref={wrapRef} className="ui-picker-wrap">
            <button
                ...
                id={id}
                type="button"
                className={`ui-picker-trigger${selected ? '' : ' is-placeholder'}${invalid ? ' is-invalid' : ''}`}
            ...
            >
                <span className="ui-picker-value">{selected ? selected.label : placeholder}</span>
