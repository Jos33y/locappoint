// analytics.js - Tracking service for sessions, events, conversions.
...
const devLog = (...args) => {
    if (!isProduction) console.log('[Analytics]', ...args)
}
...
const getOSInfo = () => {
    ...
    else if (ua.includes('Windows NT')) { os = 'Windows'; version = ua.match(/Windows NT (\d+\.\d+)/)?.[1] || '' }
    else if (ua.includes('Mac OS X')) { os = 'macOS'; version = ua.match(/Mac OS X (\d+[._]\d+)/)?.[1]?.replace('_', '.') || '' }
    else if (ua.includes('iPhone') || ua.includes('iPad')) { os = 'iOS'; version = ua.match(/OS (\d+_\d+)/)?.[1]?.replace('_', '.') || '' }
    else if (ua.includes('Android')) { os = 'Android'; version = ua.match(/Android (\d+\.?\d*)/)?.[1] || '' }
...
const getReferrerInfo = () => {
    ...
    try {
        ...
        return { referrer, referrer_domain: url.hostname }
    } catch {
...
const fetchGeoData = async () => {
    ...
    try {
        ...
        try {
            ...
            geoData = { country: null, country_name: null, region: null, city: null, timezone: null }
        }
    }
...
export const initSession = async () => {
    ...
    try {
        ...
        if (error) {
            ...
            devLog('Session initialized', sessionId)
        }
    } catch (error) {
...
// Events that signal real engagement (vs passive page/section views).
// Firing any of these flips is_bounce to false.
const INTERACTION_EVENT_TYPES = ['click', 'modal_open', 'form_submit']

export const trackEvent = async (eventType, eventName, eventCategory = null, eventValue = null, metadata = null) => {
    if (!trackingEnabled) {
        devLog('trackEvent skipped', { eventType, eventName, eventCategory, eventValue })
        return
    }
    ...
    try {
        ...
        // Real interaction implies not a bounce.
        if (INTERACTION_EVENT_TYPES.includes(eventType)) {
            await updateSession({ is_bounce: false })
        }
    } catch (error) {
...
// Recognize waitlist_step1, waitlist_step2, waitlist as waitlist conversions.
// Same prefix logic for partnership.
export const trackFormSubmit = async (formName, formData = {}) => {
    if (!trackingEnabled) {
        devLog('trackFormSubmit skipped', { formName })
        return
    }
    ...
    const isWaitlist = formName === 'waitlist' || formName.startsWith('waitlist')
    const isPartnership = formName === 'partnership' || formName.startsWith('partnership')
...
export const trackWhatsAppClick = async () => {
    ...
    await updateSession({ whatsapp_clicked: true, is_bounce: false })
}
...
export const startTimeTracking = () => {
    ...
    window.addEventListener('beforeunload', () => {
        ...
        try {
            ...
            // swallow - unload, nothing we can do
        }
    })
}
...
export const isAnalyticsEnabled = () => trackingEnabled
